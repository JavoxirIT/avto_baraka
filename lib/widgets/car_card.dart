...
carCard(context, blocState) {
  final tokenProvider = Provider.of<TokenProvider>(context);
  final languageProvider = Provider.of<LocalProvider>(context);

  if (blocState is ListingStateLoading) {
    return const Center(
      child: CircularProgressIndicator(),
    );
  }
  if (blocState is ListingStateLoad) {
    return NotificationListener<ScrollNotification>(
      onNotification: (ScrollNotification scrollInfo) {
        if (scrollInfo.metrics.pixels == scrollInfo.metrics.maxScrollExtent) {
          final currentState = blocState as ListingStateLoad;
          BlocProvider.of<ListingBloc>(context).add(ListingEventLoadMore(
      ...
      child: ListView.builder(
        itemCount: blocState.listing.length,
        itemBuilder: (context, i) {
          final item = blocState.listing[i];
  ...
  if (blocState is ListingStateNoData) {
    return Center(
      child: Text(S.of(context).malumotlatYuklanishidaXatolik),
    );
  }
  if (blocState is ListingStateError) {
    return Center(
